...
این پروژه یک دسکتاپ XFCE را با image رسمی Webtop در یک کانتینر Railway اجرا می‌کند. دسترسی گرافیکی از طریق TigerVNC/noVNC و دامنه HTTPS انجام می‌شود؛ بنابراین Firefox داخل کانتینر اجرا می‌شود، نه روی رایانه محلی. داده‌های کاربر در Volume مسیر `/config` نگهداری می‌شوند.

## معماری

`Dockerfile` از `Ubuntu 22.04 + XFCE + TigerVNC + noVNC` ساخته می‌شود. اسکریپت init، پوشه‌های کاربر و launcher Firefox را آماده می‌کند و XFCE آن را هنگام ورود اجرا می‌کند. سرویس داخلی Webtop تنها از طریق HTTPS و دامنه عمومی Railway در دسترس است و رابط مرورگر noVNC روی پورت داخلی 6080 منتشر می‌شود.
